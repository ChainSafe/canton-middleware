...
// makeOfferEvent builds a TransferOffer CREATED event with a nested transfer
// record. When executeBefore is non-zero it is included as the offer's expiry.
func makeOfferEvent(contractID string, executeBefore time.Time) *streaming.LedgerEvent {
	transferFields := map[string]streaming.FieldValue{
	...
	if !executeBefore.IsZero() {
		transferFields["executeBefore"] = streaming.MakeTimestampField(executeBefore)
	}
	fields := map[string]streaming.FieldValue{
		"transfer": streaming.MakeRecordField(transferFields),
	}
	return streaming.NewLedgerEvent(contractID, "pkg-id", transferOfferModule, transferOfferEntity, true, fields)
}

func TestOfferDecoder_Disabled_EmptyPackageID(t *testing.T) {
	decode := NewOfferDecoder("", zap.NewNop())
	ev := makeOfferEvent("offer-1", time.Time{})
	_, ok := decode(makeTx(1, ev), ev)
	assert.False(t, ok)
}

func TestOfferDecoder_Created_WithExpiry(t *testing.T) {
	decode := NewOfferDecoder("pkg-id", zap.NewNop())
	exp := time.Unix(1_800_000_000, 0).UTC()
	ev := makeOfferEvent("offer-1", exp)

	offer, ok := decode(makeTx(2, ev), ev)
	require.True(t, ok)
	assert.False(t, offer.IsArchived)
	assert.Equal(t, "offer-1", offer.ContractID)
	assert.Equal(t, testRecipient, offer.ReceiverPartyID)
	assert.Equal(t, testSender, offer.SenderPartyID)
	assert.Equal(t, testInstrumentAdmin, offer.InstrumentAdmin)
	assert.Equal(t, testInstrumentID, offer.InstrumentID)
	assert.Equal(t, testAmount, offer.Amount)
	require.NotNil(t, offer.ExpiresAt)
	assert.True(t, exp.Equal(*offer.ExpiresAt))
}

func TestOfferDecoder_Created_NoExpiry_NilPointer(t *testing.T) {
	decode := NewOfferDecoder("pkg-id", zap.NewNop())
	ev := makeOfferEvent("offer-1", time.Time{})

	offer, ok := decode(makeTx(3, ev), ev)
	require.True(t, ok)
	assert.Nil(t, offer.ExpiresAt)
}

func TestOfferDecoder_Archived_NoFields(t *testing.T) {
	decode := NewOfferDecoder("pkg-id", zap.NewNop())
	ev := streaming.NewLedgerEvent("offer-1", "pkg-id", transferOfferModule, transferOfferEntity, false, nil)

	offer, ok := decode(makeTx(4, ev), ev)
	require.True(t, ok)
	assert.True(t, offer.IsArchived)
	assert.Equal(t, "offer-1", offer.ContractID)
	assert.Nil(t, offer.ExpiresAt)
}
