...
// DEPRECATED: Use EncodeCreatePendingDepositArgs + EncodeProcessDepositAndMintArgs
func EncodeMintProposalArgs(req *MintProposalRequest) *lapiv2.Record {
	return &lapiv2.Record{
		Fields: []*lapiv2.RecordField{
			{Label: "recipient", Value: PartyValue(req.Recipient)},
			{Label: "amount", Value: NumericValue(req.Amount)},
			{Label: "txHash", Value: TextValue(req.Reference)},
		},
	}
}

// DecodeBurnEvent decodes Daml created event into BurnEvent (V1 record)
// DEPRECATED: Use DecodeWithdrawalEvent
func DecodeBurnEvent(eventID, txID string, record *lapiv2.Record) (*BurnEvent, error) {
	fields := recordToMap(record)
	...
	destination, err := extractEvmAddress(fields["destination"])
	if err != nil {
...
// DecodeBurnEventV2 decodes Daml created event into BurnEvent from V2 API record
// DEPRECATED: Use DecodeWithdrawalEventV2
func DecodeBurnEventV2(eventID, txID string, record *lapiv2.Record) (*BurnEvent, error) {
	fields := recordToMapV2(record)

	operator, err := extractPartyV2(fields["operator"])
	if err != nil {
		return nil, fmt.Errorf("failed to extract operator: %w", err)
	}

	owner, err := extractPartyV2(fields["owner"])
	if err != nil {
		return nil, fmt.Errorf("failed to extract owner: %w", err)
	}
	...
	destination, err := extractEvmAddressV2(fields["destination"])
	if err != nil {
		return nil, fmt.Errorf("failed to extract destination: %w", err)
	}

	reference, err := extractTextV2(fields["reference"])
	if err != nil {
		return nil, fmt.Errorf("failed to extract reference: %w", err)
	}
