...
func TestService_SendRawTransaction(t *testing.T) {
	...
	t.Run("success stores tx and log then returns hash", func(t *testing.T) {
		...
		pendingBlock.EXPECT().BlockNumber().Return(uint64(1))
		pendingBlock.EXPECT().BlockHash().Return(blockHashBytes)
		pendingBlock.EXPECT().SaveEvmTransaction(mock.Anything, mock.Anything).Return(nil)
		pendingBlock.EXPECT().SaveEvmLog(mock.Anything, mock.Anything).Return(nil)
	...
	t.Run("SaveEvmTransaction error propagates", func(t *testing.T) {
		...
		pendingBlock.EXPECT().BlockNumber().Return(uint64(1))
		pendingBlock.EXPECT().BlockHash().Return(blockHashBytes)
		pendingBlock.EXPECT().SaveEvmTransaction(mock.Anything, mock.Anything).Return(errors.New("save tx failed"))
		pendingBlock.EXPECT().Abort(mock.Anything).Return(nil)
		store := mocks.NewStore(t)
		store.EXPECT().NewBlock(mock.Anything, uint64(31337)).Return(pendingBlock, nil)
	...
	t.Run("SaveEvmLog error propagates", func(t *testing.T) {
		...
		pendingBlock.EXPECT().BlockHash().Return(blockHashBytes)
		pendingBlock.EXPECT().SaveEvmTransaction(mock.Anything, mock.Anything).Return(nil)
		pendingBlock.EXPECT().SaveEvmLog(mock.Anything, mock.Anything).Return(errors.New("save log failed"))
		pendingBlock.EXPECT().Abort(mock.Anything).Return(nil)
		store := mocks.NewStore(t)
		store.EXPECT().NewBlock(mock.Anything, uint64(31337)).Return(pendingBlock, nil)
