...
GO ?= $(shell command -v go 2>/dev/null || echo $(HOME)/.local/go/go1.24.13/bin/go)
...
install-mockery:
	GOBIN=$(HOME)/.local/bin $(GO) install github.com/vektra/mockery/v2@$(MOCKERY_VERSION)

generate-mocks:
	$(GO) generate -run mockery ./pkg/user/service
