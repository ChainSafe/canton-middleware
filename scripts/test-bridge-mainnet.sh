...
CHAIN_ID="${CHAIN_ID:-$CHAIN_ID_CONFIG}"
...
# Derive owner/user addresses if keys provided
if [ -n "$OWNER_KEY" ]; then
...
RELAYER_BALANCE=$(cast balance $RELAYER --rpc-url "$ETH_RPC_URL" --ether 2>/dev/null || echo "0")
print_info "Relayer ETH: $RELAYER_BALANCE"

if [ -n "$OWNER" ]; then
    OWNER_BALANCE=$(cast balance $OWNER --rpc-url "$ETH_RPC_URL" --ether 2>/dev/null || echo "0")
    print_info "Owner ETH:   $OWNER_BALANCE"
fi

if [ -n "$USER" ]; then
    USER_BALANCE=$(cast balance $USER --rpc-url "$ETH_RPC_URL" --ether 2>/dev/null || echo "0")
    print_info "User ETH:    $USER_BALANCE"
fi

# Warn if balances are low
if command -v bc &> /dev/null && [ -n "$RELAYER_BALANCE" ]; then
    if (( $(echo "$RELAYER_BALANCE < 0.1" | bc -l 2>/dev/null || echo 0) )); then
        print_warning "Relayer balance is low! May not have enough gas."
    fi
fi
...
ACTUAL_CHAIN_ID=$(cast chain-id --rpc-url "$ETH_RPC_URL" 2>/dev/null)
if [ "$ACTUAL_CHAIN_ID" != "1" ]; then
    print_error "Expected Ethereum Mainnet (chain ID 1), got chain ID $ACTUAL_CHAIN_ID"
    exit 1
fi
...
if [ -z "$OWNER_KEY" ]; then
    print_warning "Skipping bridge setup - OWNER_PRIVATE_KEY not set"
...
if [ -z "$USER_KEY" ] || [ -z "$OWNER_KEY" ]; then
    print_warning "Skipping deposit test - OWNER_PRIVATE_KEY and/or USER_PRIVATE_KEY not set"
elif [ "$DRY_RUN" = true ]; then
    print_warning "DRY RUN - Would execute deposit"
    print_info "Would transfer 1000 tokens from owner to user"
    print_info "Would approve bridge for 1000 tokens"
    print_info "Would deposit 100 tokens to Canton"
else
    # Fund user
...
if [ -z "$USER_KEY" ]; then
    print_warning "Skipping withdrawal test - USER_PRIVATE_KEY not set"
elif [ -z "$HOLDING_CID" ]; then
    print_warning "Skipping withdrawal test - No holding CID from deposit step"
elif [ "$DRY_RUN" = true ]; then
    print_warning "DRY RUN - Would execute withdrawal"
    print_info "Would withdraw 50 tokens to user address"
else
