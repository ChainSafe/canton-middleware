...
// withdraw-via-interface.go — Withdraw a USDCx TransferOffer the sender (us)
// previously created, by exercising the Splice CIP-56 TransferInstruction
// interface choice TransferInstruction_Withdraw.
//
// Mirrors accept-via-interface.go, but:
//   - the registrar endpoint is .../choice-contexts/withdraw
//   - the Daml choice is TransferInstruction_Withdraw
//   - the actor (ActAs) is the sender, reclaiming their locked input holding
//
// Use this for stale offers whose executeBefore has passed and the receiver
// never accepted.
//
// Usage:
//   go run scripts/testing/withdraw-via-interface.go \
//     -config config.api-server.devnet-test.yaml \
//     -creds ./party-credentials.txt \
//     [-cid <specific-offer-cid>]   # if omitted, withdraws every visible offer
...
const (
	...
	// Splice TransferInstruction interface (for the Withdraw choice)
...
var (
	...
	onlyCID      = flag.String("cid", "", "Optional: withdraw only this specific TransferOffer CID. If empty, withdraws every visible offer.")
...
func main() {
	...
	if *onlyCID != "" {
		...
		offers = filtered
		if len(offers) == 0 {
			fatalf("CID %s not found among visible TransferOffers for %s", *onlyCID, holder)
		}
	}
	fmt.Printf(">>> Found %d TransferOffer(s) to withdraw for %s\n", len(offers), holder)
	if len(offers) == 0 {
		return
	}
	...
	for i, cid := range offers {
		...
		regResp, raw, err := fetchWithdrawContext(ctx, httpClient, *registryHost, *registrar, cid)
		if err != nil {
		...
		if *dryRun {
			fmt.Println("  [dry-run] would exercise TransferInstruction_Withdraw")
			continue
		}

		if err := withdrawViaInterface(ctx, cantonClient, cfg, holder, kp, cid, extraArgs, disclosed); err != nil {
			fmt.Printf("  ERROR: %v\n", err)
			continue
		}
...
func fetchWithdrawContext(ctx context.Context, hc *http.Client, host, registrarParty, instructionCID string) (*acceptContextResponse, []byte, error) {
	url := fmt.Sprintf(
		"%s/api/token-standard/v0/registrars/%s/registry/transfer-instruction/v1/%s/choice-contexts/withdraw",
